...
JNIEXPORT jboolean JNICALL jnm_splash_isvisible(JNIEnv *env, jobject obj)
{
  return splashwindow_isVisible();
}

JNIEXPORT void JNICALL jnm_splash_hide(JNIEnv *env, jobject obj)
{
  splashwindow_hide();
}

JNIEXPORT void JNICALL jnm_splash_setprogress(JNIEnv *env, jobject obj, jint current, jint maximum)
{
  splashwindow_setProgress(current, maximum);
}

JNIEXPORT jobject JNICALL jnm_splash_getsplashwindow(JNIEnv *env, jobject obj, jint current, jint maximum)
{
  jclass spc = jnismooth_dll->findClass("jsmooth.SplashWindow");
  if (spc == 0)
    {
      return NULL;
    }

  jmethodID construc = jnismooth_dll->findMethod(spc, "<init>", "()V", false);
  if (construc == 0)
    {
      return NULL;
    }

  jobject splashwindow = jnismooth_dll->env()->NewObject(spc, construc);
  return splashwindow;
}
  ...
  if (nat != 0)
    {
	  ...
	  && broker->registerMethod("jsmooth.Native", "getSplashWindow", "()Ljsmooth/SplashWindow;", (void*)jnm_splash_getsplashwindow)
	  )
	{
      ...
      jclass splashc = broker->findClass("jsmooth.SplashWindow");
      if (splashc != 0)
	{
	  broker->registerMethod("jsmooth.SplashWindow", "isVisible", "()Z", (void*)jnm_splash_isvisible);
	  broker->registerMethod("jsmooth.SplashWindow", "hide", "()V", (void*)jnm_splash_hide);
	  broker->registerMethod("jsmooth.SplashWindow", "setProgress", "(II)V", (void*)jnm_splash_setprogress);
	}
    }
}
