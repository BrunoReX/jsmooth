...
public class SkeletonModel extends Observable {
	...
	public SkeletonModel(JSmoothApplication jsthModel) {
		this.skelList = new SkeletonList(new File("skeletons")); // why ? I don't know...
	...
	public int getPropertyType(SkeletonProperty prop) {
		if(prop.getType().equals(SkeletonProperty.TYPE_STRING)) return TYPE_TEXT_SINGLE;
		if(prop.getType().equals(SkeletonProperty.TYPE_TEXTAREA)) return TYPE_TEXT_MULTI;
		if(prop.getType().equals(SkeletonProperty.TYPE_BOOLEAN)) return TYPE_CHECK;
	...
	public void setPropertyValue(SkeletonProperty prop, String value) {
		System.out.println("[DEBUG] Setting property \"" +
			((SkeletonProperty) prop).getIdName() + "\" to value \"" + value + "\"");
